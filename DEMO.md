# T3000 Cameras / NVR — Windows teammate demo
...
You also need the **Microsoft Edge WebView2 Runtime** (same as HVAC graphics / Modbus Register v2). If Cameras warns that it is missing, install it from https://developer.microsoft.com/en-us/microsoft-edge/webview2/ then retry.
...
- The Cameras header shows **MediaMTX :9197** when it is up, **MediaMTX off** when it is not. Live tiles still work.
...
## 3. Open the Cameras / NVR view

1. Run **`T3000.exe`** (the one you built — it will ask for Administrator, as usual).
2. Menu: **Tools → Cameras (NVR)**.
3. A WebView2 window titled **Cameras** opens.
...
Any RTSP the Windows box can reach works (another NVR, a camera, a phone app that publishes RTSP). There is no fake “demo stream” bundled. Without a reachable URL you still see the Cameras UI and sidecar status, but no video.
...
- Sidecar panel: something like “Cameras sidecar started (go2rtc on 127.0.0.1:9191).”
...
Close **Cameras**, reopen **Tools → Cameras (NVR)** — saved cameras come back (WebView `localStorage` + `%LOCALAPPDATA%\T3000\nvr\cameras.json`).
...
2. **Tools → Cameras (NVR)** still opens a Cameras window (no crash).
...
Drop `go2rtc.exe` into one of those folders, close Cameras, open it again — no reinstall.
...
| --- | --- | --- | --- |
| go2rtc | 127.0.0.1 | **9191** | Cameras page + API |
...
If something else already owns **9191** and it is not go2rtc, Cameras opens offline and tells you the port is in use.
...
- [ ] **Tools → Cameras (NVR)** opens
- [ ] Green **go2rtc :9191**
- [ ] ONVIF **Discover** or a pasted `rtsp://...` shows **live video**
- [ ] (Optional) Hide `go2rtc.exe` and reopen Cameras — red message, T3000 still works

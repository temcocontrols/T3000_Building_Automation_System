...
// Get file buffer
int nDataSize = pHexFile->GetHexFileBuffer(pFileBuffer, c_nHexFileBufLen);//获取文件的buffer - Get file buffer
...
int check_bootloader_and_frimware(int npid ,int comport , unsigned short reg_11 , unsigned short reg_14, int &update_value, unsigned char app_already_version)
{
	...
	if ((((npid >= STM32_CO2_NET) && (npid <= STM32_PRESSURE_RS485)) ||
		(npid == STM32_PM25)) &&
		app_already_version >= 59) //不管读到没读到 boot version 都不用 再次更新boot - Regardless of whether the boot version is read or not, it does not need to be updated again
	{
			...
			(Device_infor[7] == STM32_PRESSURE_RS485))
        {
            ...
            else
            {
                ...
                else if ((Device_infor[7] == PM_MINIPANEL_ARM) && (temp_bootloader_version < 62))
                {
                    if (comport == 0)
                    {
                        c2_update_boot = false; //不支持串口更新 - Do not support serial port update
                        Ret_Result = -1;
                    }
                    else
                        c2_update_boot = true;
...
{
    //2018 0606 在底层公共读写函数增加对不同协议的处理 - Add support for different protocols in the underlying read/write functions
    if (SPECIAL_BAC_TO_MODBUS)
    {
        int n_ret = 0;
        for (int i = 0; i < retry_times; i++)
...
{
    //2018 0525 在底层公共读写函数增加对不同协议的处理 - Add support for different protocols in the underlying read/write functions
    //if ((g_protocol == MODBUS_BACNET_MSTP) || (g_protocol == PROTOCOL_MSTP_TO_MODBUS) || (g_protocol == PROTOCOL_BIP_T0_MSTP_TO_MODBUS))
    if (SPECIAL_BAC_TO_MODBUS)
    {
		retry_times = 10;

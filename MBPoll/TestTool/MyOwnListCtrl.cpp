...
void CMyOwnListCtrl::_MouseClkMonitor(UINT nMsg, UINT nFlags, CPoint point, BOOL bTriggerEdit)
{
	...
	case WM_LBUTTONDBLCLK:
		temp_item=GetItemText(hti.iItem,hti.iSubItem);
		if(temp_item.IsEmpty())
			break;
		AfxExtractSubString(temp_address,temp_item,0,'=');
		temp_address.Trim();
		g_ready_write_Address = _wtoi(temp_address);
		::PostMessage(main_hwnd,MY_OPEN_WRITE_DLG,NULL,NULL);
		//CListCtrl::OnLButtonDblClk(nFlags, point);
		g_ready_write_ID = g_read_ID;
		g_ready_write_Value= 0;
		break;

/*
 * Improved month calendar control
 * based on ReactOS Comctl32.dll library monthcal.c
 *
 ...
 * Copyright 2016 Konstantin Stukov (https://www.upwork.com/fl/havendv)
 *
...
static void MONTHCAL_PaintCalendar(const MONTHCAL_INFO *infoPtr, HDC hdc, const PAINTSTRUCT *ps, INT calIdx)
{
  ...
  SetTextColor(hdc, infoPtr->colors[MCSC_ABBREVIATIONSTEXT]);
...
MONTHCAL_SetRange(MONTHCAL_INFO *infoPtr, SHORT limits, SYSTEMTIME *range)
{
    ...
    if (limits & GDTR_MIN)
    {
		...
		infoPtr->calendars[0].month = range[0];
...
MONTHCAL_Create(HWND hwnd, LPCREATESTRUCTW lpcs)
{
  ...
  infoPtr->brushes[BrushSelected]      = CreateSolidBrush(infoPtr->colors[MCSC_SELECTEDBK]);
  infoPtr->brushes[BrushAbbreviations] = CreateSolidBrush(infoPtr->colors[MCSC_ABBREVIATIONSBK]);

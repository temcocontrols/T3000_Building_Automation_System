...
CString CppSQLite3Query::getValuebyfield(int nField)
{
	CString restr;
	checkVM();
	...
	ConvertUTF8ToString((char*)sqlite3_column_text(mpVM, nField), restr);
	return restr;
}
...
CString CppSQLite3Query::getValuebyName(CString szField, CString Defaultstring)
{
	CString restr = Defaultstring;
	int nField = fieldIndex((UTF8MBSTR)szField);
...
void CppSQLite3Query::ConvertUTF8ToString(const char* strInUTF8MB, CString& strOut)
{
	int len = (int)strlen(strInUTF8MB) + 1;


#ifdef UNICODE
	MultiByteToWideChar(CP_UTF8, 0, strInUTF8MB, len, strOut.GetBuffer(len), len);
#else
...
CString CppSQLite3Table::getValuebyName(const char* szField, const char* szNullValue/*=""*/)
{
	CString strReturn;
	if (fieldIsNull(szField))
	{
		ConvertUTF8ToString(szNullValue, strReturn);
	}
	else
	{
		ConvertUTF8ToString(fieldValue(szField), strReturn);
	}
	return strReturn;
}


void CppSQLite3Table::ConvertUTF8ToString(const char* strInUTF8MB, CString& strOut)
{
	int len = (int)strlen(strInUTF8MB) + 1;
...
bool CppSQLite3DB::tableColumnExists(const _TCHAR* szTable, const _TCHAR* szName)
{
	CppSQLite3Buffer sql;

	sql.format("SELECT count(*) FROM sqlite_master WHERE type='table' AND name = 'INPUTable'");
	int nRet = execScalar(sql);

	CString temp_cs;
	CppSQLite3Query temp_table;
	temp_cs.Format(_T("SELECT * FROM sqlite_master WHERE type='table' AND name = '%s'"), szTable);
	temp_table = execQuery((UTF8MBSTR)temp_cs);
	if (!temp_table.eof())
	{
		CString temp1;
		temp1 = temp_table.getValuebyName(L"sql");
		if (_tcsstr(temp1, szName))
		{
			return 1;
		}
		else

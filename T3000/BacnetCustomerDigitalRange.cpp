...
void CBacnetCustomerDigitalRange::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LIST_CUSTOMER_DIGITAL_RANGE, m_custom_dig_range_list);
}


BEGIN_MESSAGE_MAP(CBacnetCustomerDigitalRange, CDialogEx)
	ON_MESSAGE(WM_REFRESH_BAC_CUSTOM_DIGITAL_RANGE_LIST,Fresh_Customer_Digital_Range_List)	
...
BOOL CBacnetCustomerDigitalRange::OnInitDialog()
{
	...
	Initial_List();
	PostMessage(WM_REFRESH_BAC_CUSTOM_DIGITAL_RANGE_LIST,NULL,NULL);
...
void CBacnetCustomerDigitalRange::Initial_List()
{
	...
	m_custom_dig_range_list.GetWindowRect(list_rect);
	ScreenToClient(&list_rect);
	::GetWindowRect(m_input_dlg_hwnd,win_rect);
	m_custom_dig_range_list.Set_My_WindowRect(win_rect);
	m_custom_dig_range_list.Set_My_ListRect(list_rect);


	m_custom_dig_range_list.DeleteAllItems();
	for (int i=0;i<(int)m_custom_unit_data.size();i++)
	{
		CString temp_item,temp_dig_off,temp_dig_on,temp_dir_inv;
		temp_item.Format(_T("%d"),i+1);
		m_custom_dig_range_list.InsertItem(i,temp_item);

		for (int x=0;x<CUSTOM_DIGITAL_RANGE_COL_NUMBER;x++)
		{
			if((i%2)==0)
				m_custom_dig_range_list.SetItemBkColor(i,x,LIST_ITEM_DEFAULT_BKCOLOR);
			else
				m_custom_dig_range_list.SetItemBkColor(i,x,LIST_ITEM_DEFAULT_BKCOLOR_GRAY);		
		}
...
BOOL CBacnetCustomerDigitalRange::PreTranslateMessage(MSG* pMsg)
{
	...
	if((pMsg->message==WM_KEYDOWN && pMsg->wParam==VK_RETURN))
	{
		...
		m_custom_dig_range_list.GetWindowRect(list_rect);
		ScreenToClient(&list_rect);
		::GetWindowRect(m_custom_digital_range_dlg_hwnd,win_rect);
		m_custom_dig_range_list.Set_My_WindowRect(win_rect);
		m_custom_dig_range_list.Set_My_ListRect(list_rect);

		m_custom_dig_range_list.Get_clicked_mouse_position();
		return TRUE;
	}
...
LRESULT CBacnetCustomerDigitalRange::Fresh_Customer_Digital_Range_List(WPARAM wParam,LPARAM lParam)
{
	for (int i=0;i<(int)m_custom_unit_data.size();i++)
	{
		...
		{
			temp_dig_off.Empty();
			memset(m_custom_unit_data.at(i).digital_units_off,0,12);
		}

		if(temp_dig_on.GetLength() >=12)
		{
			temp_dig_on.Empty();
			memset(m_custom_unit_data.at(i).digital_units_on,0,12);
		}
		m_custom_dig_range_list.SetItemText(i,CUSTOM_DIGITAL_RANGE_OFF,temp_dig_off);
		m_custom_dig_range_list.SetItemText(i,CUSTOM_DIGITAL_RANGE_ON,temp_dig_on);

		CString temp_direct_inv;
		if(m_custom_unit_data.at(i).direct == 0)
		{
			temp_direct_inv = _T("Direct");
		}
		else if(m_custom_unit_data.at(i).direct == 1)
		{
		...
		m_custom_dig_range_list.SetItemText(i,CUSTOM_DIGITAL_RANGE_DIRECT_INV,temp_direct_inv);
	}
...
LRESULT CBacnetCustomerDigitalRange::Fresh_Customer_Digital_Range_Item(WPARAM wParam,LPARAM lParam)
{
	...
	memcpy_s(&m_temp_customer_unite_data[Changed_Item],sizeof(Str_Units_element),&m_custom_unit_data.at(Changed_Item),sizeof(Str_Units_element));

	CString temp_task_info;
	CString New_CString =  m_custom_dig_range_list.GetItemText(Changed_Item,Changed_SubItem);
	CString cstemp_value;
	if(Changed_SubItem == CUSTOM_DIGITAL_RANGE_OFF)
	{
		CString cs_temp = m_custom_dig_range_list.GetItemText(Changed_Item,Changed_SubItem);
		...
		memcpy_s(m_custom_unit_data.at(Changed_Item).digital_units_off,12,cTemp1,12);
	}

	if(Changed_SubItem == CUSTOM_DIGITAL_RANGE_ON)
	{
		CString cs_temp = m_custom_dig_range_list.GetItemText(Changed_Item,Changed_SubItem);
	    //The length cannot be greater than the length defined by the structure;
		if(cs_temp.GetLength()>= 12)	//长度不能大于结构体定义的长度;
		...
		memset(cTemp1,0,255);
		WideCharToMultiByte( CP_ACP, 0, cs_temp.GetBuffer(), -1, cTemp1, 255, NULL, NULL );
		memcpy_s(m_custom_unit_data.at(Changed_Item).digital_units_on,12,cTemp1,12);
	}


	cmp_ret = memcmp(&m_temp_customer_unite_data[Changed_Item],&m_custom_unit_data.at(Changed_Item),sizeof(Str_Units_element));
	if(cmp_ret!=0)
	{
		m_custom_dig_range_list.SetItemBkColor(Changed_Item,Changed_SubItem,LIST_ITEM_CHANGED_BKCOLOR);
		temp_task_info.Format(_T("Write Input List Item%d .Changed to \"%s\" "),Changed_Item + 1,New_CString);
		Post_Write_Message(g_bac_instance,WRITEUNIT_T3000,Changed_Item,Changed_Item,sizeof(Str_Units_element),m_custom_digital_range_dlg_hwnd ,temp_task_info,Changed_Item,Changed_SubItem);
	}
...
void CBacnetCustomerDigitalRange::OnNMClickListCustomerDigitalRange(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	long lRow,lCol;
	m_custom_dig_range_list.Set_Edit(true);
	DWORD dwPos=GetMessagePos();//Get which line is click by user.Set the check box, when user enter Insert it will jump to program dialog
	CPoint point( GET_X_LPARAM(dwPos), GET_Y_LPARAM(dwPos));
	m_custom_dig_range_list.ScreenToClient(&point);
	LVHITTESTINFO lvinfo;
	lvinfo.pt=point;
	lvinfo.flags=LVHT_ABOVE;
	int nItem=m_custom_dig_range_list.SubItemHitTest(&lvinfo);
	...
	if(lRow>m_custom_dig_range_list.GetItemCount()) //如果点击区超过最大行号，则点击是无效的
		return;
	if(lRow<0)
		return;
	CString New_CString;
	...
	if(lCol == CUSTOM_DIGITAL_RANGE_DIRECT_INV)
	{
		memcpy_s(&m_temp_customer_unite_data[lRow],sizeof(Str_Units_element),&m_custom_unit_data.at(lRow),sizeof(Str_Units_element));

		if(m_custom_unit_data.at(lRow).direct == 0)
		{

			m_custom_unit_data.at(lRow).direct = 1;
			m_custom_dig_range_list.SetItemText(lRow,CUSTOM_DIGITAL_RANGE_DIRECT_INV,_T("Invers"));
			New_CString = _T("Invers");
		}
		else
		{
			m_custom_unit_data.at(lRow).direct = 0;
			m_custom_dig_range_list.SetItemText(lRow,CUSTOM_DIGITAL_RANGE_DIRECT_INV,_T("Direct"));
	...
	m_custom_dig_range_list.Set_Edit(false);

	int cmp_ret = memcmp(&m_temp_customer_unite_data[lRow],&m_custom_unit_data.at(lRow),sizeof(Str_Units_element));
	if(cmp_ret!=0)
	{
		m_custom_dig_range_list.SetItemBkColor(lRow,lCol,LIST_ITEM_CHANGED_BKCOLOR);
		temp_task_info.Format(_T("Write Custom Digital Units Item%d .Changed to \"%s\" "),lRow + 1,New_CString);
		Post_Write_Message(g_bac_instance,WRITEUNIT_T3000,(int8_t)lRow,(int8_t)lRow,sizeof(Str_Units_element),m_custom_digital_range_dlg_hwnd,temp_task_info,lRow,lCol);
	}

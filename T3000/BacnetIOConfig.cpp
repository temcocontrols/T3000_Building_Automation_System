...
LRESULT CBacnetIOConfig::Fresh_Extio_List(WPARAM wParam,LPARAM lParam)
{
	...
	for (int i=0;i<(int)m_extio_config_data.size();i++)
	{
		...
		CString temp_in;CString temp_out;
		...
		//m_ext_io_config_list.SetItemText(i,EXTIO_LAST_CONTACT,_T(""));
...
void CBacnetIOConfig::CaculateIOCount(int nitem)
{
	...
	for (int i=nitem;i<temp_list_count;i++)
	{
		...
		int pid_sequence = 0;
		//if(temp_pidtype.CompareNoCase(ExtIO_Product[0]) == 0)
		//{
		//	pid_sequence = 0;
		//}
		//else if(temp_pidtype.CompareNoCase(ExtIO_Product[1]) == 0)
		//{
		//	pid_sequence = 1;
		//}

...
LRESULT CBacnetTstat::Fresh_Tstat_List(WPARAM wParam, LPARAM lParam)
{
	...
	for (int i = 0; i < (int)m_Tstat_data.size(); i++)
	{
		...
		CString ModuleName;
		CString Address;
		CString RoomTemperature;
		CString SerialNumber;
		CString OCCUPIED;
		CString Cool_Setpoint;
		CString Heat_Setpoint;
		CString Room_Setpoint;
		CString CS_TSTAT_MODE;
		CString CS_COOL_HEAT;
		CString CS_OUTPUT_STAGE;

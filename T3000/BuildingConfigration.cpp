...
void CBuildingConfigration::RemoteVPNDisConnectivity()
{


    //system("runas cmd.exe");
    system("cmd.exe /c wireguard.exe/uninstalltunnelservice configfile");
...
void CBuildingConfigration::Initial_Building_List()
{
    ...
    for (int i = 0; i < (int)m_BuildNameLst.size(); i++)
    {
        ...
        if (ListCtrlEx::ComboBox == m_building_config_list.GetColumnType(BC_STATUS))
        {
            ListCtrlEx::CStrList strlist;
            for (int j = 0; j < m_szBuildingComs.size(); j++)
            {
            ...
            m_building_config_list.SetCellStringList(i, BC_STATUS, strlist);
        }
        if (ListCtrlEx::EditBox == m_building_config_list.GetColumnType(BC_Config))
        {
            ListCtrlEx::CStrList strlist;
            for (int j = 0; j < m_szBuildingComs.size(); j++)
            {
            ...
            m_building_config_list.SetCellStringList(i, BC_Config, strlist);
        }
...
void CBuildingConfigration::Fresh_List_Row()
{
    ...
    if (ListCtrlEx::ComboBox == m_building_config_list.GetColumnType(BC_STATUS))
    {
        ...
        m_building_config_list.SetCellStringList(last_new_item, BC_STATUS, strlist);
    }
...
void CBuildingConfigration::Update_Building()
{
    ...
    BuildingTemp.BaudRate = m_building_config_list.GetItemText(m_changedRow, BC_STATUS);
    BuildingTemp.BaudRate = m_building_config_list.GetItemText(m_changedRow, BC_Config);
    try
    {
        CString strSql;
...
LRESULT CBuildingConfigration::Fresh_Building_Config_Item(WPARAM wParam, LPARAM lParam)
{
    ...
    else if (Changed_SubItem == BC_PROTOCOL)
    {
        ...
        if (cs_temp.CompareNoCase(_T("Auto")) == 0)    //只有填写远程的时候需要填写IP地址和端口;
        {
            ...
            (cs_temp.CompareNoCase(_T("Remote System")) == 0))
        {
            ...
            if (cs_temp.CompareNoCase(_T("Remote System")) == 0)
            {
    ...
    else if (Changed_SubItem == BC_IPADDRESS)
    {
        ...
        if (temp_protocol.CompareNoCase(_T("Remote System")) == 0)
        {
...
void CBuildingConfigration::OnBnClickedBuildingButtonAdd()
{
    ...
    if (strProtocol.CompareNoCase(_T("Auto")) == 0)
    {
        ...
        (strProtocol.CompareNoCase(_T("Remote System")) == 0))
    {
...
void CBuildingConfigration::OnNMClickListBuildingConfig(NMHDR* pNMHDR, LRESULT* pResult)
{
    ...
    if (BC_Config == lCol)
    {
        ...
        if (IDOK == dlg.DoModal())
        {
            ...
            m_building_config_list.SetItemText(lRow, lCol, filepath);
...
void CBuildingConfigration::OnLvnItemchangedListBuildingConfig(NMHDR* pNMHDR, LRESULT* pResult)
{

...
#include "CBacnetProtocol.h"

using namespace Scintilla;

// Find the first run at a position
int CBacnetProtocol::RunFromPosition(int position) const {
...
int CBacnetProtocol::SplitRun(int position) {
...
void CBacnetProtocol::RemoveRun(int run) {
	starts->RemovePartition(run);
	styles->DeleteRange(run, 1);
}

void CBacnetProtocol::RemoveRunIfEmpty(int run) {
...
void CBacnetProtocol::RemoveRunIfSameAsPrevious(int run) {
...
CBacnetProtocol::CBacnetProtocol() {
...
int CBacnetProtocol::Length() const {
	return starts->PositionFromPartition(starts->Partitions());
}

int CBacnetProtocol::ValueAt(int position) const {
	return styles->ValueAt(starts->PartitionFromPosition(position));
}

int CBacnetProtocol::FindNextChange(int position, int end) const {
...
int CBacnetProtocol::StartRun(int position) const {
	return starts->PositionFromPartition(starts->PartitionFromPosition(position));
}

int CBacnetProtocol::EndRun(int position) const {
	return starts->PositionFromPartition(starts->PartitionFromPosition(position) + 1);
}

bool CBacnetProtocol::FillRange(int &position, int value, int &fillLength) {
	if (fillLength <= 0) {
		return false;
	}
...
void CBacnetProtocol::SetValueAt(int position, int value) {
	int len = 1;
	FillRange(position, value, len);
}

void CBacnetProtocol::InsertSpace(int position, int insertLength) {
	int runStart = RunFromPosition(position);
	if (starts->PositionFromPartition(runStart) == position) {
		int runStyle = ValueAt(position);
...
void CBacnetProtocol::DeleteAll() {
	starts.reset(new Partitioning(8));
	styles.reset(new SplitVector<int>());
	styles->InsertValue(0, 2, 0);
}

void CBacnetProtocol::DeleteRange(int position, int deleteLength) {
...
int CBacnetProtocol::Runs() const {
	return starts->Partitions();
}

bool CBacnetProtocol::AllSame() const {
...
bool CBacnetProtocol::AllSameAs(int value) const {
	return AllSame() && (styles->ValueAt(0) == value);
}

int CBacnetProtocol::Find(int value, int start) const {
	if (start < Length()) {
...
void CBacnetProtocol::Check() const {
	if (Length() < 0) {
		throw std::runtime_error("CBacnetProtocol: Length can not be negative.");
	}
	if (starts->Partitions() < 1) {
		throw std::runtime_error("CBacnetProtocol: Must always have 1 or more partitions.");
	}
	if (starts->Partitions() != styles->Length()-1) {
		throw std::runtime_error("CBacnetProtocol: Partitions and styles different lengths.");
	}
	int start=0;
	while (start < Length()) {
		const int end = EndRun(start);
		if (start >= end) {
			throw std::runtime_error("CBacnetProtocol: Partition is 0 length.");
		}
		start = end;
	}
	if (styles->ValueAt(styles->Length()-1) != 0) {
		throw std::runtime_error("CBacnetProtocol: Unused style at end changed.");
	}
	for (int j=1; j<styles->Length()-1; j++) {
		if (styles->ValueAt(j) == styles->ValueAt(j-1)) {
			throw std::runtime_error("CBacnetProtocol: Style of a partition same as previous.");
		}
	}
}

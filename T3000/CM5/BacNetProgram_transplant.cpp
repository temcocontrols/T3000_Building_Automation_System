		...
		case KEYWORD:
			...
			case Alarm:
				...
				if ((p = (char *)strchr(prog, '<')) != NULL)
					c = LT;
				else if ((p = (char *)strchr(prog, '>')) != NULL)
					c = GT;
				*p = ',';
...
int pcodvar(int cod, int v, char* var, float fvar, char* op, int Byte)
{
	...
	if (op != NULL)
		while (op[i] && i < MAX_OP)
		{
			switch (op[i])
			{
			case '+': cod_line[Byte++] = PLUS;
				break;
			case '-': cod_line[Byte++] = MINUS;
				break;
			case '*': cod_line[Byte++] = MUL;
				break;
			case '/': cod_line[Byte++] = DIV;
				break;
			case '%': cod_line[Byte++] = MOD;
				break;
			case '^': cod_line[Byte++] = POW;
				break;
			case '&': cod_line[Byte++] = BIT_AND;
				break;
			case '|': cod_line[Byte++] = BIT_OR;
				break;
			case LT:
			case LE:
			case GT:
		...
		case Alarm:
			...
			*/
			...
			if ((p = (char *)memchr(code, LT, 30)) != NULL)
				c = LT;
			else if ((p = (char *)memchr(code, GT, 30)) != NULL)
				c = GT;
			if (p == NULL)
			{
				error = 1;
...
int desvar(void)
{
	...
	else if (((unsigned char)*code) == 0x9c/*(byte)*//*LOCAL_POINT_PRG*/)
	{
		//#ifdef Fance_recode
	...
	else if (((unsigned char)*code) == (byte)REMOTE_POINT_PRG)
	{
		++code;
	...
	else if (((unsigned char)*code) == CONST_VALUE_PRG)
	{
		float f;
		long l;
		memcpy(&l, ++code, 4);

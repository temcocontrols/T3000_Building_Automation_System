// CTransducer.cpp: 实现文件 - Implementation file
//
...
extern int CANVAS_TOP_X ;  //画布 - Canvas
extern int CANVAS_TOP_Y;
extern int CANVAS_BOTTOM_X ;
extern int CANVAS_BOTTOM_Y ;
extern int ORIGIN_X ; //原点 - Origin point
extern int ORIGIN_Y ;

extern int Y_AXIS_TOP_X ;  //Y轴最上面点 - Y-axis top point
extern int Y_AXIS_TOP_Y ;
extern int Y_AXIS_ARROW_LEFT_X ;  //Y轴的箭头左边 - Left side of Y-axis arrow
extern int Y_AXIS_ARROW_LEFT_Y ;
extern int Y_AXIS_ARROW_RIGHT_X ; // Y轴的箭头右边 - Right side of Y-axis arrow
extern int Y_AXIS_ARROW_RIGHT_Y ;
extern int X_AXIS_RIGHT_X ;  //X轴最右边  X坐标 - X-axis rightmost X coordinate
extern int X_AXIS_RIGHT_Y ;				//X轴最右边  Y坐标 - X-axis rightmost Y coordinate
extern int X_AXIS_MAX_VALUE_X ; //X轴最大点  X坐标 - X-axis maximum point X coordinate
extern int X_AXIS_MAX_VALUE_Y ; //X轴最大点  Y坐标 - X-axis maximum point Y coordinate
extern int Y_AXIS_MAX_VALUE_X ;
extern int Y_AXIS_MAX_VALUE_Y ;
extern int X_AXIS_ARROW_TOP_X ; //X轴的箭头上边 - Top of X-axis arrow
extern int X_AXIS_ARROW_TOP_Y ;
extern int X_AXIS_ARROW_BOTTOM_X ; //X轴的箭头下边 - Bottom of X-axis arrow
...
//版本号大于25时启用新寄存器 - Enable new registers when version number is greater than 25
...
// CTransducer 诊断 - CTransducer diagnostics
...
// CTransducer 消息处理程序 - CTransducer message handlers
...
void CTransducer::OnInitialUpdate()
{
	...
	// TODO: 在此添加专用代码和/或调用基类 - TODO: Add specialized code here and/or call the base class
}


void CTransducer::InitialPaintParameter()
{
     CANVAS_TOP_X = 550 + 50;  //画布 - Canvas
	 CANVAS_TOP_Y = 0 + 50;
	 CANVAS_BOTTOM_X = 1210 + 50;
	 CANVAS_BOTTOM_Y = 420 + 100;
	 ORIGIN_X = 600 + 50; //原点 - Origin point
	 ORIGIN_Y = 400 + 50;
	 Y_AXIS_TOP_X = ORIGIN_X;  //Y轴最上面点 - Y-axis top point
	 Y_AXIS_TOP_Y = CANVAS_TOP_Y + 10;
	 Y_AXIS_ARROW_LEFT_X = Y_AXIS_TOP_X - 5;  //Y轴的箭头左边 - Left side of Y-axis arrow
	 Y_AXIS_ARROW_LEFT_Y = Y_AXIS_TOP_Y + 10;
	 Y_AXIS_ARROW_RIGHT_X = Y_AXIS_TOP_X + 5; // Y轴的箭头右边 - Right side of Y-axis arrow
	 Y_AXIS_ARROW_RIGHT_Y = Y_AXIS_TOP_Y + 10;

	 X_AXIS_RIGHT_X = CANVAS_BOTTOM_X - 10;  //X轴最右边  X坐标 - X-axis rightmost X coordinate
	 X_AXIS_RIGHT_Y = ORIGIN_Y;				//X轴最右边  Y坐标 - X-axis rightmost Y coordinate

	 X_AXIS_MAX_VALUE_X = X_AXIS_RIGHT_X - 100; //X轴最大点  X坐标 - X-axis maximum point X coordinate
	 X_AXIS_MAX_VALUE_Y = X_AXIS_RIGHT_Y; //X轴最大点  Y坐标 - X-axis maximum point Y coordinate

	 Y_AXIS_MAX_VALUE_X = Y_AXIS_TOP_X;
	 Y_AXIS_MAX_VALUE_Y = Y_AXIS_TOP_Y + 50;

	 X_AXIS_ARROW_TOP_X = X_AXIS_RIGHT_X - 10; //X轴的箭头上边 - Top of X-axis arrow
	 X_AXIS_ARROW_TOP_Y = X_AXIS_RIGHT_Y - 5;
	 X_AXIS_ARROW_BOTTOM_X = X_AXIS_RIGHT_X - 10; //X轴的箭头下边 - Bottom of X-axis arrow
...
void CTransducer::Fresh()
{
	...
	CStatic* pWnd = (CStatic*)GetDlgItem(IDC_STATIC_TRANSDUCER_TEMPERATURE); // 得到 Picture Control 句柄 - Get Picture Control handle ;
	...
	//设置静态控件窗口风格为位图居中显示 - Set static control window style to center bitmap display
	p->ModifyStyle(0xf, SS_BITMAP | SS_CENTERIMAGE);
	//将图片设置到Picture控件上 - Set the image to the Picture control
	p->SetBitmap(bitmap);
	...
	{
		is_new_firmware = true; //大于等于25版本的 Xducer  寄存器都变位置了 - For Xducer version 25 or above, register positions have all changed ;
	}
	else
		is_new_firmware = false; //大于等于25版本的 Xducer  寄存器都变位置了 - For Xducer version 25 or above, register positions have all changed ;
...
void CTransducer::OnPaint()
{
	CPaintDC dc(this); // device context for painting
					   // TODO: 在此处添加消息处理程序代码 - TODO: Add message handler code here
					   // 不为绘图消息调用 CFormView::OnPaint() - Do not call CFormView::OnPaint() for painting messages
	...
	if (is_new_firmware)
	{
		...
		mygraphics->DrawLine(myRectangle_pen, Y_AXIS_TOP_X, Y_AXIS_TOP_Y, Y_AXIS_ARROW_LEFT_X, Y_AXIS_ARROW_LEFT_Y); //画 Y轴 箭头 - Draw Y-axis arrow
		mygraphics->DrawLine(myRectangle_pen, Y_AXIS_TOP_X, Y_AXIS_TOP_Y, Y_AXIS_ARROW_RIGHT_X, Y_AXIS_ARROW_RIGHT_Y);

		mygraphics->DrawLine(myRectangle_pen, X_AXIS_RIGHT_X, X_AXIS_RIGHT_Y, X_AXIS_ARROW_TOP_X, X_AXIS_ARROW_TOP_Y); //画 X轴 箭头 - Draw X-axis arrow
...
#pragma region for_hum
		...
		PointF      point_line1;
		point_line1.X = ORIGIN_X + 100; //画图下面的标识线 ，标识温度还是湿度 - Draw identification line below the graph to indicate temperature or humidity
		...
		point_line1.X = ORIGIN_X + 300;//画图下面的标识线 ，标识温度还是湿度 - Draw identification line below the graph to indicate temperature or humidity
		point_line1.Y = CANVAS_BOTTOM_Y - 20;

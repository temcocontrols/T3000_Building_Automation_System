	...
	void CGridFlashDlg::DoDataExchange(CDataExchange* pDX)
	{
		...
		DDX_Control(pDX, IDC_MATCH, m_MatchBtn);
	}
	...
	BEGIN_MESSAGE_MAP(CGridFlashDlg, CDialog)
		...
		ON_BN_CLICKED(IDCANCEL2, &CGridFlashDlg::OnBnClickedCancel2)
		ON_BN_CLICKED(IDC_MATCH, &CGridFlashDlg::OnBnClickedMatch)
	END_MESSAGE_MAP()
  ...
  m_pBackCheckThread->m_bAutoDelete=FALSE;
	...
	{
		if (m_EnableSaveBtn.IsWindowEnabled())
	...
	void CGridFlashDlg::OnBnClickedCancel()
	{
		...
		if (!m_EnableSaveBtn.IsWindowEnabled())
		{
			AfxMessageBox(_T("Flashing,Please Waiting...."));
		} 
		else
		{
			CDialog::OnCancel();
...
//从第一个开始 到最后一个 
	...
	//-------------FOR Begin------------------------//
			...
			if(pDlg->m_grid_flash.at(iitemp).flash_or_no==true)
			{		
...
////////////////////////////////////begin
	...
	//////////////Flash-Begin
						...
						if(pDlg->m_grid_flash.at(iitemp).software_version==p_mode)
						{
							...
							if(nEpSize==1)
							{
								int ntemp2=read_one(now_tstat_id,11);//ISP version
								if (ntemp2<34)
								{
									//The bootloader firmware  can only accommodate firmware files up to rev 29.8. 
									//The product needs to be returned to the factory in order to be updated for firmware greater than version 29.8.
									bSaveconfigFile=false;

									CString strTemp,strInfo;
									strTemp.LoadString(IDS_LOADERINFPSTR);
									strInfo.Format(_T("%s,are you sure you want to burn this file?"),strTemp);
									CAfxMessageDialog dlg;
									dlg.SetPromtionTxt(strInfo);
									if(dlg.DoModal()==IDOK)
									{
							...
							else
							{
								...
								///The E2 memory chip is too small to accommodate hex file version greater than version 29.8.
								bSaveconfigFile=false;
								CString strTemp,strInfo;
								strTemp.LoadString(IDS_E2PROMTSTR);
								strInfo.Format(_T("%s,are you sure you want to burn this file?"),strTemp);
						...
						else//normal model; //end of P model
						{
							/////Flash-begin-CRC错误后，从这里继续烧写
							//int nSpecialValue=read_one(pDlg->m_grid_flash.at(iitemp).ID,326);//MODBUS_EEPROM_SIZE,
							//if(nSpecialValue==1)
							//{
							//	write_one(pDlg->m_grid_flash.at(iitemp).ID,324,0);//MODBUS_CONFIGURATION
							//}
							//int nret;
							//nret=write_one(now_tstat_id,321,4);//MODBUS_FAN_GRIDPOINT
							//nret=write_one(now_tstat_id,322,0);//MODBUS_MODE_GRIDPOINT
							//nret=write_one(now_tstat_id,323,0);//MODBUS_HOLD_GRIDPOINT
							//nret=write_one(now_tstat_id,325,0);//MODBUS_UTIL_MODE
							//if(pDlg->m_grid_flash.at(iitemp).device == PM_NC)//NC不可以用
							//{		
							//	failure_number++;
							//	pDlg->m_FlexGrid.put_TextMatrix(iitemp+1,10,_T("FAIL"));
							//	continue;
							...
							//int ntemp=0;
							//now_tstat_id=pDlg->m_grid_flash.at(iitemp).ID;//get temp tstat id	
							//write_one(now_tstat_id,16,1);// 
							//ntemp=read_one(now_tstat_id,326);//MODBUS_EEPROM_SIZE
							//if (ntemp==1)
							//{
							//	int ntemp2=-1;

							//	write_one(now_tstat_id,16,127);////go to isp mode //TBD: need a header file to show proper ,meaning of the registers
							//	bool bRet = Change_BaudRate(19200);
							//	Sleep(1000);

							//	ntemp2=read_one(now_tstat_id,11);//ISP version
							//	if (ntemp2<34)
							//	{
							//		//The bootloader firmware  can only accommodate firmware files up to rev 29.8. 
							//		//The product needs to be returned to the factory in order to be updated for firmware greater than version 29.8.
					...
					while(NULL!=m_hex_file.ReadString(m_get_data_from_file))
					{
						...
						if(get_hex[3]==1)
							//for to seektobegin() function,because to end of the file
							break;
					...
					//	CString save_file_path;//path of the save file							
					//------------------------------------------------------------save-----------------------------------------------------
	//				long serialnumber_temp;
	//				bool bool_serial=get_serialnumber(serialnumber_temp);//bool_serialnumber true==success,false == failure
					...
					if(can_be_writed_hex_file(pDlg->m_grid_flash.at(iitemp).device,register_data[0x100]))
					{
						...
						{
                             showing_text.Format(_T("Restart,please waiting..."));
						 	Sleep(13000);
                             showing_text.Format(_T("Start Finished"));	
							//wait for the tstat restart  
							/*when Flash Finished,must write Register 7 103
							                                            Register 8   5*/
							 
                             showing_text.Format(_T("Write Device Name--Begin"));	
                           
							{
								// 要求 LCD--5G  
								//int device=0;
								//device=read_one(now_tstat_id,7);//当启动好了以后，就可以读到7寄存器的值，如果读不到，说明还没有重启好，继续等待
								//while(device<=0)
								//{
								//	device=read_one(now_tstat_id,7);
								//}
								//write_one(now_tstat_id,7,103);
								//write_one(now_tstat_id,8,5);
							}
							/*Test Can Read The Value From Register 7  .it is 103*/
				/*			int id=read_one(now_tstat_id,7);
							CString str;
							str.Format(_T("%d"),id);
							AfxMessageBox(str);*/
                          showing_text.Format(_T("Write Device Name--End"));	
...
//----------------Flash----end
	...
	void CGridFlashDlg::OnBnClickedCancel2()
	{
		 OnCancel();
	}

	void CGridFlashDlg::OnBnClickedMatch()
	{
	    // AfxMessageBox(_T("Before Flashing,Judge the Product Model!"));
	}

...
extern CBacnetArray* Array_Window;
...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWndEx)
    ...
    ON_COMMAND(ID_CONTROL_PANEL, &CMainFrame::OnControlArray)
...
void CMainFrame::OnHTreeItemSeletedChanged(NMHDR* pNMHDR, LRESULT* pResult)
{
    ...
    for (UINT i = 0; i < m_product.size(); i++)
    {
        if (hSelItem == m_product.at(i).product_item)
        {
            ...
            else if (m_product.at(i).product_class_id == PM_T332AI_ARM) //T3
            {
                g_tstat_id = m_product.at(i).product_id;
                DoConnectToANode(hSelItem);
                break;
...
void CMainFrame::LoadProductFromDB()
{
    ...
    if (!q.eof())
    {
        while (!q.eof())//所有 设备;
        {
            ...
            GetExtProductInfo(m_product_temp, temp_ext);
    ...
    if (hlocalnetwork != NULL)
        m_pTreeViewCrl->Expand(hlocalnetwork, TVE_EXPAND);
...
LRESULT CMainFrame::HandleWriteNewDevice(WPARAM wParam, LPARAM lParam)
{
    int original_serial = int(wParam);
    int des_serial = int (lParam);
        ...
        for (temp_it = m_product.begin(); temp_it != m_product.end(); ++temp_it)
        {
            if (temp_it->serial_number == des_serial)
            {
                DoConnectToANode(temp_it->product_item);
                break;
            }
        }
        SetPaneString(BAC_SHOW_MISSION_RESULTS, _T("Writing data, please wait"));
        Sleep(5000);
        CString temp_serial;
        temp_serial.Format(_T("%u"), original_serial);
        CString temp_path;
        //temp_path = g_strExePth + _T("Database\\temp\\") + temp_serial +_T(".prog");
...
void CMainFrame::DoConnectToANode(const HTREEITEM& hTreeItem)
{
	...
	for (UINT i = 0; i < m_product.size(); i++)
	{
		if (hSelItem == m_product.at(i).product_item)
		{
			...
			if (m_product.at(i).m_ext_info.virtual_device == 1)
			{
				offline_mode = true;
				set_offline_mode(offline_mode);
				...
				)	//如果是CM5或者MINIPANEL 才有 bacnet协议;
			{
				...
				else if (selected_product_Node.protocol == PROTOCOL_BIP_T0_MSTP_TO_MODBUS)
				{
					...
					g_protocol = PROTOCOL_BACNET_IP;
				if (selected_product_Node.m_ext_info.virtual_device)
					offline_mode = true;
				else
					offline_mode = false;
				SwitchToPruductType(DLG_BACNET_VIEW);
...
void CMainFrame::CheckDeviceParameter()
{
    ...
    for (int i = 0; i < m_refresh_net_device_data.size(); i++)
    {
        ...
        if (m_refresh_net_device_data.at(i).object_instance >= BACNET_MAX_INSTANCE)
        {
            find_ip_conflict = true;
			device_id_data_1 = m_refresh_net_device_data.at(i);
			edit_confilct_mode = true;
			::PostMessageW(MainFram_hwd, WM_HADNLE_DUPLICATE_ID, CONFILIC_DEVICE_BACNET_INSTANCE, NULL);
			return;
        }
    }
}
...
UINT _FreshTreeView(LPVOID pParam )
{
    ...
    while(1)
    {
        ...
        if ((b_pause_refresh_tree) || (b_building_management_flag != SYS_NORMAL_MODE))
        {
            if ((debug_item_show == DEBUG_SHOW_ALL) || (debug_item_show == DEBUG_SHOW_SCAN_ONLY))
            {
...
void CMainFrame::OnControlMain()
{
    ...
    else
    {
        ...
        else if (product_type == CS3000||product_register_value[7]==PM_T322AI || product_register_value[7] == PM_T332AI_ARM || product_register_value[7] == PWM_TRANSDUCER ||product_register_value[7]==PM_T38AI8AO6DO || product_register_value[7]==PM_T3PT12
...
void CMainFrame::OnControlArray()
{
    ...
    if ((g_protocol == PROTOCOL_BACNET_IP) || (g_protocol == MODBUS_BACNET_MSTP) || (g_protocol == PROTOCOL_BIP_TO_MSTP))
    {
        ...
        if (pDialog[WINDOW_ARRAY] != NULL)
        {
            ...
            AnnualRoutine_Window->Unreg_Hotkey();
            Monitor_Window->Unreg_Hotkey();
            if (pDialog[WINDOW_ARRAY]->IsWindowVisible() == false)
            {
                Array_Window->Reset_Array_Rect();
                pDialog[WINDOW_ARRAY]->ShowWindow(SW_SHOW);
            }
            ((CDialogCM5_BacNet*)m_pViews[DLG_BACNET_VIEW])->m_bac_main_tab.SetCurSel(13);
            Array_Window->m_array_list.SetFocus();
            bacnet_view_number = TYPE_ARRAY;
            g_hwnd_now = m_array_dlg_hwnd;

            CString temp_ui;
            temp_ui.Format(_T("%u"), TYPE_ARRAY);
            WritePrivateProfileString(_T("LastView"), _T("FistLevelViewUI"), temp_ui, g_cstring_ini_path);
        }
        ...
        if (g_protocol == PROTOCOL_BIP_TO_MSTP)
        {
            Create_Thread_Read_Item(TYPE_ARRAY);
        }
        else
        {
...
void CMainFrame::OnControlRefresh()
{
...
BOOL CMainFrame::OnToolTipNotify(UINT id,NMHDR *Pnmhdr,LRESULT *pResult)
{
    ...
    if (Pnmhdr->hwndFrom == m_testtoolbar.Get_Tip_HWND())
    {
        ...
        if (pBtn)
        {
            ...
            else if (pBtn->m_nID == ID_CONTROL_REFRESH)
            {
...
void CMainFrame::Reset_Window_Pos()
{
	...
	if(b_create_status)
	{
        ...
        {
            if (((CBacnetArray*)pDialog[WINDOW_ARRAY])->IsWindowVisible())
            {
                ((CBacnetArray*)pDialog[WINDOW_ARRAY])->Reset_Array_Rect();
            }
        }
...
void CMainFrame::VirtualDeviceMode()
{
   // b_building_management_flag = SYS_VIRTUAL_MODE;
...
int CMainFrame::DoConnectDB_TreeNode(const HTREEITEM& hTreeItem)
{
    if (b_building_management_flag)
    {
        for (UINT i = 0; i < m_product.size(); i++)
        {
            if (hTreeItem == m_product.at(i).product_item)
            {
                return 0;
            }
        }

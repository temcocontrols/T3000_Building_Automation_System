...
_RecordsetPtr& COperateDB::OpenRecordset( CString sql )
{
	ASSERT(!sql.IsEmpty());
	try 
	{
...
void COperateDB::CloseRecordset()
{
...
UINT COperateDB::GetRecordCount( _RecordsetPtr pRecordset )
{
	int nCount = 0;
	try 
	{
...
void COperateDB::Createtable( CString strSQL )
{
	COperateDB m_ado;
	m_ado.OnInitADOConn();
...
bool COperateDB::IsHaveTable( COperateDB ado, CString strTableName )
{

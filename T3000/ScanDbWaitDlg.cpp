	...
	if(nIDEvent == 1)
	{
		...
		{		
			if(!m_pScaner->IsAllScanFinished())
			{
				...
				switch(icon_count)	
				{
				case 0:  GetDlgItem(IDC_STATIC_SCAN_PIC)->SetWindowPos(NULL,Scan_rect.left,Scan_rect.top,0,0,SWP_NOZORDER|SWP_NOSIZE); break;
				case 3: GetDlgItem(IDC_STATIC_SCAN_PIC)->SetWindowPos(NULL,Scan_rect.left + 5,Scan_rect.top + 5,0,0,SWP_NOZORDER|SWP_NOSIZE); break;
				case 6: GetDlgItem(IDC_STATIC_SCAN_PIC)->SetWindowPos(NULL,Scan_rect.left + 10,Scan_rect.top + 10,0,0,SWP_NOZORDER|SWP_NOSIZE);break;
				case 9: GetDlgItem(IDC_STATIC_SCAN_PIC)->SetWindowPos(NULL,Scan_rect.left + 5,Scan_rect.top + 15,0,0,SWP_NOZORDER|SWP_NOSIZE);break;
				case 12: GetDlgItem(IDC_STATIC_SCAN_PIC)->SetWindowPos(NULL,Scan_rect.left,Scan_rect.top + 20,0,0,SWP_NOZORDER|SWP_NOSIZE); break;
				case 15:  GetDlgItem(IDC_STATIC_SCAN_PIC)->SetWindowPos(NULL,Scan_rect.left -5,Scan_rect.top +15,0,0,SWP_NOZORDER|SWP_NOSIZE); break;
				case 18: GetDlgItem(IDC_STATIC_SCAN_PIC)->SetWindowPos(NULL,Scan_rect.left -10,Scan_rect.top +10,0,0,SWP_NOZORDER|SWP_NOSIZE); break;
				case 21: GetDlgItem(IDC_STATIC_SCAN_PIC)->SetWindowPos(NULL,Scan_rect.left -5,Scan_rect.top +5,0,0,SWP_NOZORDER|SWP_NOSIZE);break;
				default:break;
				}
...
void CScanDbWaitDlg::Initial_List()
{
	...
	for (int i=0;i<ncount;i++)
	{
		...
		for (int x=0;x<SCAN_ALL_COLUME;x++)
		{
				m_scan_com_list.SetItemBkColor(i*2 +1,x,LIST_ITEM_DEFAULT_BKCOLOR_GRAY);
				m_scan_com_list.SetItemBkColor(i*2 + 2,x,LIST_ITEM_DEFAULT_BKCOLOR);		
	...
	m_scan_com_list.InsertItem(ncount*2 + 1,scan_mode);
	//m_scan_com_list.SetItemText(i*2,SCAN_BAUDRATE,_T("9600"));
	m_scan_com_list.SetItemText(ncount*2 + 1,SCAN_SKIP,_T("No"));
	m_scan_com_list.SetItemText(ncount*2 + 1,SCAN_STATUS,_T("Wait"));
	m_scan_com_list.SetItemText(ncount*2 + 1,SCAN_FOUND,_T("0"));

	scan_mode = _T("Remote Device");
	m_scan_com_list.InsertItem(ncount*2 + 2,scan_mode);
	m_scan_com_list.SetItemText(ncount*2 + 2,SCAN_SKIP,_T("No"));
	m_scan_com_list.SetItemText(ncount*2 + 2,SCAN_STATUS,_T("Wait"));
	m_scan_com_list.SetItemText(ncount*2 + 2,SCAN_FOUND,_T("0"));

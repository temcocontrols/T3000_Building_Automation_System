...
HDC status_hMemDC;
HBITMAP status_hBmp; // For double buffering - 用于双缓冲;
CRect status_myRect;
...
DWORD WINAPI MyStatusBarThread(LPVOID lPvoid)
{
	...
	while (b_statusbarthreadflag)
	{
		...
		BitBlt(status_gloab_hdc, 0, 0, status_myRect.right, status_myRect.bottom, status_hMemDC, 0, 0, SRCCOPY);// Paste the completed memory bitmap to the Picture control object - 将绘制完成的内存位图贴到的Picture空间对象中;
		for (int z = 0; z < 100; z++)
		{
...
void CMyStatusbarCtrl::DrawStatusBar(HDC my_hdc)
{
	...
	if (g_llTxCount > old_tx_count) //每发送一次才进来一次; - Only enter once for each transmission
	{
    ...
    delete HealthGreenBrush;  //2018 03 02 fandu添加 - Added by fandu on 2018 03 02
    delete HealthRedBrush;    //2018 03 02 fandu添加 - Added by fandu on 2018 03 02
}
...
{
    // TODO: 在此添加消息处理程序代码和/或调用默认值 - Add message handler code and/or call default values here

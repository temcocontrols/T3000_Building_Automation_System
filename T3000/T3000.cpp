...
const unsigned int g_versionNO = 20250513;    // PROJECT_VERSION
...
//#ifdef _DEBUG
    char strTime[128] = { 0 }; // 鍙栧皬鏃跺綋 灏忕増鏈彿;
    CString Test_Version;  //   TIME 鍜孌ATE    
...
void CT3000App::UpdateDB()
{
    ...
    if (is_update)
    {
        ...
        remove((UTF8MBSTR)g_strDatabasefilepath);//鍒犳帀鍘熸湁鐨勬暟鎹簱

        CString FilePath;
        HANDLE hFind;
        WIN32_FIND_DATA wfd;
        hFind = FindFirstFile(g_strDatabasefilepath, &wfd);//
        if (hFind==INVALID_HANDLE_VALUE)//涓嶅瓨鍦ㄨ鏂囦欢
        {
...
BOOL CT3000App::InitInstance()
{
	...
	//	Logger::WriteMessage("T3000 Unit Tester Begin....");
		...
		try
		{
			...
			CString www_zip_file; //鍒朵綔瀹夎鏂囦欢鐨勬椂鍊欏皢瀹夎鏂囦欢鐨剒ip鏂囦欢瑙ｅ帇鍒皐ww鏂囦欢澶逛笅
			...
			if (hFind_Monitor==INVALID_HANDLE_VALUE)//璇存槑褰撳墠鐩綍涓嬫棤MonitorData.db
			{
				//没有找到就创建一个默认的数据库

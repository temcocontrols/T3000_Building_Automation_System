...
#if 0//ÅÐ¶ÏÍøÕ¾ÊÇ·ñÓÐÐÂ°æ±¾£¬ÍøÂçÁ¬½Ó²»ÊÇ»òÕßÃ»ÓÐÐÂ°æ±¾£¬Ö±½Ó½øÈë
...
{
	LPCTSTR   pszDllName   =   OcxFileName   ;			//ActiveX¿Ø¼þµÄÂ·¾¶¼°ÎÄ¼þÃû       
	HINSTANCE   hLib   =   LoadLibrary(pszDllName);    //×°ÔØActiveX¿Ø¼þ   
	if   (hLib   <   (HINSTANCE)HINSTANCE_ERROR)   
	{   
		return   FALSE;   
	}   
	FARPROC   lpDllEntryPoint;     
	lpDllEntryPoint   =   GetProcAddress(hLib,(LPCSTR)(_T("DllRegisterServer")));     //»ñÈ¡×¢²áº¯ÊýDllRegisterServerµØÖ·   
	if(lpDllEntryPoint!=NULL)														 //µ÷ÓÃ×¢²áº¯ÊýDllRegisterServer   
	{   
...
// 			}
			...
			CBADO tempado;
			tempado.SetDBPath(g_achive_monitor_datatbase_path);	//ÔÝÊ±²»´´½¨ÐÂÊý¾Ý¿â
			tempado.OnInitADOConn(); 


			if (tempado.IsHaveTable(tempado,_T("Version")))//ÓÐVersion±í
			{
...
// 		m_lastinterface= 19 ;//GetPrivateProfileInt(_T("T3000_START"),_T("Interface"),19,g_configfile_path); //ÓÉ ¶Å·« 16-03-17ÆÁ±Î£»½øÈë²»Í¬µÄ½çÃæÒýÆð T3000´ò¿ªÊ±±¨´í¡£
...
// 		}
	...
	catch (...)
	{
     ...
     //g_strDatabasefilepath   =g_strExePth+_T("Database\\T3000.db");
		...
		//Õâ¸öÒªÏÈÊÔÊÔ£¬µ±µçÄÔÃ»ÓÐ°²×°Õâ¸öÎÄ¼þÊ±£¬ÈçºÎ²¶»ñÕâ¸öÐÅÏ¢£¬È»ºóÔÙÖ´ÐÐÕâ¸ö¡£
...
//	strPath+=_T("T3000.exe");
	...
	timeSpan=curTime-fileStatus.m_mtime;//Õâ¸öÊÇÐÞ¸ÄÊ±¼ä£¬²»ÂÛÄãCUT£¬COPY¶¼²»»áÐÞ¸ÄµÄ

...
void T3000PRESSURE_NET::RegisterWndClass()
{
...
BEGIN_MESSAGE_MAP(T3000PRESSURE_NET, CControlBar)
	//{{AFX_MSG_MAP(T3000PRESSURE_NET)
	ON_WM_ERASEBKGND()
	ON_WM_SIZE()
	ON_WM_NCPAINT()
	ON_WM_DESTROY()
...
/////////////////////////////////////////////////////////////////////////////
// T3000PRESSURE_NET message handlers

//-----------------------------------------------------------------------------

CSize T3000PRESSURE_NET::CalcFixedLayout(BOOL stretch, BOOL horz)
{
	CSize bar_size;
...
CSize T3000PRESSURE_NET::CalcDynamicLayout(int length, DWORD mode)
{
	return CalcLayout(mode, length);
}


CSize T3000PRESSURE_NET::CalcLayout(DWORD mode, int length)
{
...
void T3000PRESSURE_NET::SizeToolBar(int length, bool vert/*= false*/)
{
	if (vert)
...
//-----------------------------------------------------------------------------
void T3000PRESSURE_NET::OnNcPaint()
{
	EraseNonClient();
}


void T3000PRESSURE_NET::OnUpdateCmdUI(CFrameWnd* target, BOOL disable_if_no_hndler)
{}

//-----------------------------------------------------------------------------
static const TCHAR* title_= _T("Dynamic Help");

bool T3000PRESSURE_NET::Create(CWnd* parent_wnd, UINT id)
{
...
BOOL T3000PRESSURE_NET::OnEraseBkgnd(CDC* dc)
{
	if (!IsFloating())
	{
		CRect rect;
...
void T3000PRESSURE_NET::DoPaint(CDC* dc)
{
	CRect rect;
	GetClientRect(rect);
...
BOOL T3000PRESSURE_NET::OnToolTipGetText(UINT uId, NMHDR* nm_hdr, LRESULT* result)
{
...
void T3000PRESSURE_NET::OnSize(UINT type, int cx, int cy)
{
...
void T3000PRESSURE_NET::OnDestroy()
{
...
int T3000PRESSURE_NET::OnCreate(LPCREATESTRUCT create_struct)
{
...
void T3000PRESSURE_NET::SetContextHelp(const TCHAR* text, const TCHAR* header/*= 0*/)
{
	CString text= _T(
...
void T3000PRESSURE_NET::DisplayHelp(const CString& line, int word_start, int word_end)
{
	CString help;
...
void T3000PRESSURE_NET::OnBarStyleChange(DWORD old_style, DWORD new_style)
{
	PostMessage(WM_USER);
}

LRESULT T3000PRESSURE_NET::OnDelayedResize(WPARAM, LPARAM)
{
	Resize();
	return 0;
}
...
void T3000PRESSURE_NET::OnCloseWnd()
{

...
BOOL CVirtualProgramEngine::ExecuteInstruction(ProgramContext& ctx)
{
    ...
    switch (opcode) {
        ...
        case LOCAL_POINT_PRG: {
            ...
            float value = ReadVariable(0, pointType, 0, number);
            ctx.valueStack.push(value);
            break;
        }
        ...
        default: {
            ...
            if (opcode >= 0x30 && opcode <= 0xD3) {
                float result = GetTimeFunc(opcode);
...
BOOL CVirtualProgramEngine::ExecIf(ProgramContext& ctx)
{
    ...
    // Read else target address
    int elseIp = ReadShort(code + ctx.ip);
    ctx.ip += 2;
...
BOOL CVirtualProgramEngine::ExecFor(ProgramContext& ctx)
{
    ...
    // Read loop variable index
    unsigned char varIndex = ReadByte(code + ctx.ip); ctx.ip++;
    ...
    // Read step (optional, default 1)
    float step = 1.0f;
    if (ctx.valueStack.size() >= 1) {
        step = ctx.valueStack.top(); ctx.valueStack.pop();
    }
    
    // Set loop variable
    if (varIndex < (int)ctx.localVariables.size()) {
    ...
    // Read target address
    loop.targetIp = ReadShort(code + ctx.ip);
    ctx.ip += 2;
...
{
    if (ctx.valueStack.empty()) {
        SetError(ctx, "StartPrg: stack empty");
        return FALSE;
    }
...
BOOL CVirtualProgramEngine::ParseOperand(const unsigned char* ip, ProgramContext& ctx, float& outValue, int& bytesConsumed)
{
    ...
    switch (marker) {
        ...
        case LOCAL_POINT_PRG: {
            ...
            outValue = ReadVariable(0, pointType, 0, number);
            bytesConsumed = (int)(p - ip);
            return TRUE;
        }

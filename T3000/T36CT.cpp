...
void T36CT::OnInitialUpdate()
{
    ...
    // Color display
    //彩色显示
    for(int i=1;i<4+1;i++)		// Row count - 排数量
    {
        for(int k=0;k<4;k++)	// Column count - 列数量
        {
            if (i%2==1)
            {
    ...
    // Interface Output DO section list box
    //============================================================================================================界面Output DO部份列表框
...
//    CString str;
    ...
    //彩色显示 - Color display
    for(int i=1;i<6;i++)		//排数量 - Row count
    {

        for(int k=0;k<2;k++)	//列数量 - Column count
        {
            if (i%2==1)
            {
...
void T36CT::ClickMsflexgridOutput()
{
    grid_flag=3;
    long lRow,lCol;
    lRow = m_msflexgrid_output.get_RowSel();//获取点击的行号	- Get clicked row number
    lCol = m_msflexgrid_output.get_ColSel(); //获取点击的列号 - Get clicked column number
    TRACE(_T("Click input grid!\n"));

    CRect rect;
    m_msflexgrid_output.GetWindowRect(rect); //获取表格控件的窗口矩形 - Get the window rectangle of the grid control
    ...
    //形成选中个所在的矩形区域 - Calculate the rectangle area where the selected cell is located
    ...
    if(lRow!=0)
    {
        ...
        if (m_curcol==0||m_curcol==2)
        {
            ...
            m_inNameEdt.SetSel(nLenth,nLenth); //全选// - Select all
        } 
        //else 
        //{
...
void T36CT::ClickMsflexgridInput()
{
   grid_flag=1;
    ...
    long lRow,lCol;
    lRow = m_msflexgrid_input.get_RowSel();//获取点击的行号	- Get clicked row number
    lCol = m_msflexgrid_input.get_ColSel(); //获取点击的列号 - Get clicked column number
    TRACE(_T("Click input grid!\n"));
    ...
    if(2==lCol)
    {
        ...
        m_comboxRange.SetFocus(); //获取焦点 - Get focus
        m_comboxRange.SetWindowText(strValue);
    }
    else  
    {
...
void T36CT::ClickMsflexgridInput2()
{
    ...
    long lRow,lCol;
    lRow = m_msflexgrid_Ainput.get_RowSel();//获取点击的行号	- Get clicked row number
    lCol = m_msflexgrid_Ainput.get_ColSel(); //获取点击的列号 - Get clicked column number
    TRACE(_T("Click input grid!\n"));

    CRect rect;
    m_msflexgrid_Ainput.GetWindowRect(rect); //获取表格控件的窗口矩形 - Get the window rectangle of the grid control
    ScreenToClient(rect); //转换为客户区矩形 - Convert to client area rectangle
    CDC* pDC =GetDC();

...
void T38AI8AO::OnInitialUpdate()
{
	...
	// Color display
	//彩色显示
	for(int i=1;i<INPUT_ROWS+1;i++)		// Row count - 排数量
	{

		for(int k=0;k<INPUT_COLS;k++)	// Column count - 列数量
		{
			if (i%2==1)
			{
	...
	// Color display
	//彩色显示
	for(int i=1;i<OUTPUT_ROWS+1;i++)		// Row count - 排数量
	{

		for(int k=0;k<OUTPUT_COLS;k++)	// Column count - 列数量
		{
			if (i%2==1)
			{
...
bPauseMultiRead=TRUE;
	m_isinput=FALSE;
	long lRow,lCol;
	lRow = m_msflexgrid_output.get_RowSel();//获取点击的行号	- Get the clicked row number
	lCol = m_msflexgrid_output.get_ColSel(); //获取点击的列号 - Get the clicked column number
	...
	if(lCol==1&&lRow!=0)
	{
		//return; // 2012.2.7老毛说不允许修改 - 2012.2.7 Lao Mao said modification is not allowed
		m_inNameEdt.MoveWindow(&rcCell,1);
		m_inNameEdt.ShowWindow(SW_SHOW);
		m_inNameEdt.SetWindowText(strValue);
		m_inNameEdt.SetFocus();

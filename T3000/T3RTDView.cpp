...
CT3RTDView::~CT3RTDView()
{
    ...
    //pMain->m_pRefreshThread->ResumeThread();	// Cannot add this section, otherwise it will error when exiting T3000 - 不能加这一段，否则 退出T3000的时候 报错; Fance comments
...
//#if 1	
    ...
    // Color display
    //彩色显示
    for(int i=1;i<10+1;i++)		// Row count - 排数量
    {
        for(int k=0;k<6;k++)	// Column count - 列数量
        {
            if (i%2==1)
            {

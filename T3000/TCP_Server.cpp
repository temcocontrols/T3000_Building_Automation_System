...
void CALLBACK Listen(SOCKET s, int ServerPort, const char *ClientIP)
{
	...
	while(!bTimeOut)//!pScanner->m_bNetScanFinish)  // 超时结束
	{
		...
		//nRet = ::sendto(s,(char*)pSendBuf,nSendLen,0,(sockaddr*)&h_bcast_temp,sizeof(h_bcast_temp));
...
void CTCP_Server::Initial_List()
{
	...
	//m_tcp_server_list.InsertItem(0,scan_mode);
	...
	case ADD_ITEM:
		{
			...
			//temp_serialnumber.Format(_T("%u"),temp_client->serialnumber);
			//temp_port.Format(_T("%d"),temp_client->ipport);
			//list_count = m_tcp_server_list.GetItemCount(); //temp_client->IPAddress
			//temp_item.Format(_T("%d"),list_count);
			//m_tcp_server_list.InsertItem(list_count,temp_item);
			//m_tcp_server_list.SetItemText(list_count,TCP_IP_ADDRESS,temp_client->IPAddress);
			//m_tcp_server_list.SetItemText(list_count,TCP_IP_PORT,temp_port);
			//m_tcp_server_list.SetItemText(list_count,TCP_PRODUCT_NAME,temp_client->Product_name);
			//m_tcp_server_list.SetItemText(list_count,TCP_SERIALNUMBER,temp_serialnumber);

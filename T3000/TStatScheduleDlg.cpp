...
const int SCHED_START_ADDR= 357;
...
void CTStatScheduleDlg::InitGrid()
{
	...
	m_fgWeekend.put_TextMatrix(0,4,_T("Time Setting"));	
...
void CTStatScheduleDlg::FillGridByRegister()
{
	for (int i = 1; i < 5; i++)
	{
		...
		int nHour = multi_register_value[nWorkDayAwakeH+(i-1)*2];
		int nMin = multi_register_value[nWorkDayAwakeM+(i-1)*2];
	//	int nSec = multi_register_value[1];
		...
		strTime = strHour+_T(":")+strMin+_T("  ")+strPMAM;
...
// 		int nSec2 = multi_register_value[1];
		...
		m_fgWorkDay.put_TextMatrix(i,4,strTime);	

		//////////////////////////////////////////////////////////////////////////
		int nWeekenAwakeH = m_szRegTable[SCHED_WEEKEN_AWAKE_HOUR]; 
		int nWeekenAwakeM = m_szRegTable[SCHED_WEEKEN_AWAKE_MIN]; 
		nHour = multi_register_value[nWeekenAwakeH+(i-1)*2];
		nMin = multi_register_value[nWeekenAwakeM+(i-1)*2];
		...
		m_fgWeekend.put_TextMatrix(i,3,strPMAM);	
...
// 		}
	...
	if (m_bInWorkDay)
	{
		...
		CString strTime = strHour+_T(":")+strMin+_T("  ")+strPMAM;	
		m_fgWorkDay.put_TextMatrix(m_szEditPosInWork.cx, 4, strTime);
	}
	else
	{
		...
		CString strTime = strHour+_T(":")+strMin+_T("  ")+strPMAM;	
		m_fgWeekend.put_TextMatrix(m_szEditPosInWeekend.cx, 4, strTime);
...
{	
	...
	for (int i = 1; i < 5; i++)
	{
		...
		strPMAM = m_fgWorkDay.get_TextMatrix(i,3);	
...
void CTStatScheduleDlg::InitRegisterTable()
{
	...
	if (multi_register_value[7] == PM_TSTAT6 || multi_register_value[7] == PM_TSTAT7 ) // TStat 6
	{
		for (int i = 0 ; i < 16; i++)
		{
			m_szRegTable.push_back(357 + i);
		}		
	}
// 	else if(multi_register_value[7] == 27) // TStat 7
// 	{
// 		for (int i = 0 ; i < 16; i++)
// 		{
// 			m_szRegTable.push_back(357 + i);

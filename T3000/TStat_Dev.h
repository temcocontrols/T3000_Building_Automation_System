...
// TStat5 系列设备的基类 - Base class for TStat5 series devices
class CTStat_Dev :
	public CTStatBase
{
public:
	...
	// 也必须通过读写寄存器来实现 - Must also be implemented through reading and writing registers
...
public:
	int				m_nprotocol;				//协议 - Protocol
	int				m_nBaudRate;				// 波特率 - Baud rate
	int				m_nComPort;				// 串口 - Serial port	
	int				m_nEPSize;					// ??
	//int				m_nAddrID;				// 地址ID，表示在ModBus中的地址==不需要，就是devID - Address ID, represents address in ModBus == not needed, just devID
};

...
:m_nBaudRate(0),				// 波特率 - Baud rate
m_nComPort(0),				// 串口 - Serial port	
m_dwIPAddr(0),				// IP Address
m_nPort(0)					// 当前端口 - Current port
//CTStatBase::m_nProductType(100)				// 产品ID - Product ID
{
...
// 也必须通过读写寄存器来实现,这个函数需要实时的反映在线状态 - Must also be implemented through reading and writing registers, this function needs to reflect online status in real time
...
//----------------------------------------------------------------
// 以下两个才需要去读写寄存器 - The following two need to read and write registers
...
CString CTStat_Net::GetProductName()
{
	...
	strID.Format(_T("%u"), m_dwSerialID);//20120424  //scan 系列号总多出l - Scan serial number always has more than l

	CString strDevID;
	//if(m_nprotocol == PROTOCOL_BACNET_IP)//如果是bacnet ip - If it is bacnet ip
	//{

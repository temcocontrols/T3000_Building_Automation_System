...
#if 1	
	//设置排/行数量 - Set row/column count
	m_sensor_table.put_Cols(3);
	m_sensor_table.put_Rows(11);//包括标题栏 - Including header
	//显示横标题 - Show horizontal header
...
{
	if (product_register_value[454] == 0)//说明是选中状态 - Indicates selected state
	{
...
{
	if (product_register_value[454] == 1)//说明是选中状态 - Indicates selected state
	{

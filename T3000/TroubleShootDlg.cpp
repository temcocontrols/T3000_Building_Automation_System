...
#pragma endregion new_socket
		...
		if( -1 == bind(h_scan_Broad,(SOCKADDR*)&h_siBind,sizeof(h_siBind)))//把网卡地址强行绑定到Socket - Force bind network card address to Socket  
		{
...
// 		FD_SET(h_scan_Broad, &fdSocket);
	...
	//	Sleep(3000);
		...
		if(nSelRet > 0)
		{
			...
			{		
				FD_ZERO(&fdSocket);
				if(buffer[0]==0x67)//收到正确的回复了 - Received correct reply
				{	

...
bool CTstat8HelySystem::operator <(const CTstat8HelySystem &other) const {
...
bool SelectionRange::Contains(CTstat8HelySystem sp) const {
	if (anchor > caret)
...
bool SelectionRange::Trim(SelectionRange range) {
	const CTstat8HelySystem startRange = range.Start();
	const CTstat8HelySystem endRange = range.End();
	CTstat8HelySystem start = Start();
	CTstat8HelySystem end = End();
...
Selection::Selection() : mainRange(0), moveExtends(false), tentativeMain(false), selType(selStream) {
	AddSelection(SelectionRange(CTstat8HelySystem(0)));
}
...
CTstat8HelySystem Selection::Start() const {
	if (IsRectangular()) {
...
CTstat8HelySystem Selection::Last() const {
	CTstat8HelySystem lastPosition;
	for (const SelectionRange &range : ranges) {

...
class CTstat8HelySystem {
	Sci::Position position;
	Sci::Position virtualSpace;
public:
	explicit CTstat8HelySystem(Sci::Position position_=INVALID_POSITION, Sci::Position virtualSpace_=0) : position(position_), virtualSpace(virtualSpace_) {
	...
	bool operator ==(const CTstat8HelySystem &other) const {
...
struct SelectionSegment {
	CTstat8HelySystem start;
	CTstat8HelySystem end;
	SelectionSegment() : start(), end() {
	}
	SelectionSegment(CTstat8HelySystem a, CTstat8HelySystem b) {
		if (a < b) {
			start = a;
			end = b;
		} else {
	...
	void Extend(CTstat8HelySystem p) {
		if (start > p)
			start = p;
		if (end < p)
			end = p;
...
struct SelectionRange {
	CTstat8HelySystem caret;
	CTstat8HelySystem anchor;

	SelectionRange() : caret(), anchor() {
	}
	explicit SelectionRange(CTstat8HelySystem single) : caret(single), anchor(single) {
	}
	explicit SelectionRange(Sci::Position single) : caret(single), anchor(single) {
	}
	SelectionRange(CTstat8HelySystem caret_, CTstat8HelySystem anchor_) : caret(caret_), anchor(anchor_) {
	}

...
char* desassembler_program(char* input_code,char *output_text);
...
//fance pedit->length_code=code-pedit->code;
	...
	*pmes = 0;
	...
	memcpy(encode_str->encode_array, mycode, my_lengthcode);
	
end_encode:
	if (error == -1)
	{
		encode_str->errorcode = -1;
		strcpy(encode_str->encode_message, "encode succss!");
	}
	else
	{
		encode_str->errorcode = error;
		strcpy(encode_str->encode_message, pmes);
	}
	
	return error; //if -1  means no error;
}	/* end of main */
...
char* pcode, * ptimebuf;
char* local;
int bac_program_size; //extern int bac_program_size;
int bac_free_memory;//extern int bac_free_memory;
char* desassembler_program( char * input_code, decode_str* decodestr)
{
...
//									 adjustint(&n, ptrprg->type);
	...
	*buf = 0;
	...
	strcpy(decodestr->decode_text, my_display);
end_decode:
	if (error != -1)
	{
		decodestr->errorcode = error;
		strcpy(decodestr->decode_message, "ERROR!!Desassambler!!");
		decodestr->decode_text[0] = 0;
		return NULL;
	}

	return decodestr->decode_text;
}
...
char * Decode_function(char* input_code, decode_str* decodestr)
{
	...
	ret_results = desassembler_program(input_code, decodestr);
	return ret_results;
}

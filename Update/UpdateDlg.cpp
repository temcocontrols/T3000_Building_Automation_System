...
CString cs_new_name; //自定义软件的名字; - Custom Software Name
bool run_t3000 = true;
...
bool CUpdateDlg::DeleteDirectory(CString DirName)
{
    ...
    while (IsFinded)
    {
        ...
        if (!tempFind.IsDots())
        {
            ...
            {
                SetFileAttributes(strDirName, FILE_ATTRIBUTE_NORMAL); //去掉文件的系统和隐藏属性 - Remove system and hidden attributes
...
void CUpdateDlg::InitialTemcoLogo()
{
    ...
    CStatic *pStatic = (CStatic *)GetDlgItem(IDC_STATIC_T3000_UPDATE); //获得指向静态控件的指针 - Get pointer to static control
    pStatic->ModifyStyle(0xF, SS_BITMAP/*|SS_CENTERIMAGE*/);  //设置静态控件的样式，使其位图居中 - Set static control style to center bitmap
    pStatic->SetBitmap(hBitmap);     //设置静态控件显示位图  - Set static control to display bitmap
}
...
{
    CreateDirectory(strDstPath, NULL);// 创建目标文件夹，如果存在则不创建，如果不存在则创建。- Create the destination folder, do not create if it exists, create if it does not exist.
    if (PathIsDirectory(strDstPath) && PathFileExists(strSrcPath))//目标目录是否存在 而且strSrcPath 要么是文件夹要么是文件 - Check if the destination directory exists and strSrcPath is either a folder or a file
    {
        if (::PathIsDirectory(strSrcPath))//如果是文件夹拷贝 - If it is a folder copy
        {
            CFileFind finder;
            // 打开指定的文件夹进行搜索 - Open the specified folder for searching
            BOOL bWorking = finder.FindFile(strSrcPath + _T("\\") + _T("*.*"));
            while (bWorking)
            {
                // 从当前目录搜索文件 - Search files from the current directory
                bWorking = finder.FindNextFile();
                CString strFileName = finder.GetFileName();
                CString strSrc = strSrcPath + _T("\\") + strFileName;
                CString strDst = strDstPath + _T("\\") + strFileName;
                // 判断搜索到的是不是"."和".."目录 - Check if the found directory is not "." and ".."
                if (!finder.IsDots())
                {
                    // 判断搜索到的目录是否是文件夹 - Check if the found directory is a folder
                    if (finder.IsDirectory())
                    { // 如果是文件夹的话，进行递归 - If it is a folder, perform recursion
                        if (!CopyDirW(strSrc, strDst, bFailIfExists))
                            return false;
                    }
                    else
                    {// 如果是文件，进行复制 - If it is a file, perform copy
        ...
        else//如果是文件拷贝 - If it is a file copy
        {
            strDstPath += _T("\\");
    ...
    else//目的路径不存在 - Destination path does not exist
        return false;
    return true;
...
BOOL KillProcessFromName(CString strProcessName)
{
    //创建进程快照(TH32CS_SNAPPROCESS表示创建所有进程的快照) - Create a process snapshot (TH32CS_SNAPPROCESS indicates creating a snapshot of all processes)

    HANDLE hSnapShot = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);



    //PROCESSENTRY32进程快照的结构体 - PROCESSENTRY32 is the structure for process snapshots

    PROCESSENTRY32 pe;



    //实例化后使用Process32First获取第一个快照的进程前必做的初始化操作 - Initialization before using Process32First to get the first snapshot process
    ...
    //下面的IF效果同 - The following IF effect is the same as

    //if(hProcessSnap == INVALID_HANDLE_VALUE)   无效的句柄 - Invalid handle
    ...
    //将字符串转换为小写 - Convert the string to lowercase

    strProcessName.MakeLower();



    //如果句柄有效  则一直获取下一个句柄循环下去 - If the handle is valid, keep getting the next handle in a loop
...

    {
        ...
        //pe.szExeFile获取当前进程的可执行文件名称 - pe.szExeFile gets the executable file name of the current process
        ...
        //将可执行文件名称所有英文字母修改为小写 - Convert all letters in the executable file name to lowercase

        scTmp.MakeLower();



        //比较当前进程的可执行文件名称和传递进来的文件名称是否相同 - Compare the executable file name of the current process with the passed file name

        //相同的话Compare返回0 - If they are the same, Compare returns 0
...

        {
            ...
            //从快照进程中获取该进程的PID(即任务管理器中的PID) - Get the PID of the process from the snapshot process (i.e., the PID in Task Manager)
...
DWORD WINAPI GetFtpFileThread(LPVOID lPvoid)
{
    ...
    if (is_local_temco_net == FALSE)
    {
        ...
        int retry_count = 0;
        DeleteUrlCacheEntry(T3000FtpPath); // 清理缓存 - Clear cache
        Sleep(2000);
        while (1)
        {
    ...
    //这里要检测是否下载完毕; - Here we need to check if the download is complete;
...
BOOL CUpdateDlg::DownloadFileHttp(const CString& strFileURLInServer, //待下载文件的URL - URL of the file to be downloaded
    const CString & strFileLocalFullPath)//存放到本地的路径 - Path to store the file locally
{
    ...
    session.SetOption(INTERNET_OPTION_CONNECT_TIMEOUT, nTimeOut); //重试之间的等待延时 - Wait delay between retries
    session.SetOption(INTERNET_OPTION_CONNECT_RETRIES, 1);   //重试次数 - Number of retries
    char* pszBuffer = NULL;
    ...
    try
    {
        ...
        if (dwStateCode == HTTP_STATUS_OK)
        {
            ...
            char szInfoBuffer[1000];  //返回消息 - Return message
            DWORD dwFileSize = 0;   //文件长度 - File length
            ...
            pszBuffer = new char[BUFFER_LENGTH];  //读取文件的缓冲 - Buffer for reading the file
            DWORD dwWrite, dwTotalWrite;
            dwWrite = dwTotalWrite = 0;
            UINT nRead = pHttpFile->Read(pszBuffer, BUFFER_LENGTH); //读取服务器上数据 - Read data from the server

            while (nRead > 0)
            {
                WriteFile(hFile, pszBuffer, nRead, &dwWrite, NULL);  //写到本地文件 - Write to local file
